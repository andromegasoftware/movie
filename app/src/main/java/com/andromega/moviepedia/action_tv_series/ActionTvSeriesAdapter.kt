...
class ActionTvSeriesAdapter (var movieList: List<ActionTvSeriesModelClass>, val clickListener: (ActionTvSeriesModelClass) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var circularMovies: List<ActionTvSeriesModelClass> = ArrayList()
    ...
    fun submitList(circularMovieList: List<ActionTvSeriesModelClass>){
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(actionTvSeriesModelClass: ActionTvSeriesModelClass, clickListener: (ActionTvSeriesModelClass) -> Unit){
            movieName.text = actionTvSeriesModelClass.MovieName
            movieIdmb.text = itemView.resources.getString(R.string.average_vote) + actionTvSeriesModelClass.voteAverage
            movieYear.text = itemView.resources.getString(R.string.main_page_year) + actionTvSeriesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + actionTvSeriesModelClass.picture
            Picasso.get().load(pictureUrl).error(R.drawable.avenger).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(actionTvSeriesModelClass) }
        }
    }
}