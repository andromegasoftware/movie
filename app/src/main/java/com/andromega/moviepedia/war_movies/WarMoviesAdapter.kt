...
class WarMoviesAdapter (var movieList: List<WarMoviesModelClass>, val clickListener: (WarMoviesModelClass) -> Unit): RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var movies: List<WarMoviesModelClass> = ArrayList()
    ...
    fun submitList(movieList: List<WarMoviesModelClass>){
        movies = movieList
    }
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(warMoviesModelClass: WarMoviesModelClass, clickListener: (WarMoviesModelClass) -> Unit){
            movieName.text = warMoviesModelClass.title
            movieIdmb.text = "IDMB: " + warMoviesModelClass.voteAverage
            movieYear.text = "YEAR: " + warMoviesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + warMoviesModelClass.posterPath
            Picasso.get().load(pictureUrl).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(warMoviesModelClass) }
        }
    }
}