...
class HistoryMoviesAdapter (var movieList: List<HistoryMoviesModelClass>, val clickListener: (HistoryMoviesModelClass) -> Unit): RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var movies: List<HistoryMoviesModelClass> = ArrayList()
    ...
    fun submitList(movieList: List<HistoryMoviesModelClass>){
        movies = movieList
    }
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(historyMoviesModelClass: HistoryMoviesModelClass, clickListener: (HistoryMoviesModelClass) -> Unit){
            movieName.text = historyMoviesModelClass.title
            movieIdmb.text = itemView.resources.getString(R.string.average_vote) + historyMoviesModelClass.voteAverage
            movieYear.text = itemView.resources.getString(R.string.main_page_year) + historyMoviesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + historyMoviesModelClass.posterPath
            Picasso.get().load(pictureUrl).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(historyMoviesModelClass) }
        }
    }
}