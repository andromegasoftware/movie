...
class ComedyMoviesAdapter (var movieList: List<ComedyMoviesModelClass>, val clickListener: (ComedyMoviesModelClass) -> Unit): RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var movies: List<ComedyMoviesModelClass> = ArrayList()
    ...
    fun submitList(movieList: List<ComedyMoviesModelClass>){
        movies = movieList
    }
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(comedyMoviesModelClass: ComedyMoviesModelClass, clickListener: (ComedyMoviesModelClass) -> Unit){
            movieName.text = comedyMoviesModelClass.title
            movieIdmb.text = itemView.resources.getString(R.string.average_vote) + comedyMoviesModelClass.voteAverage
            movieYear.text = itemView.resources.getString(R.string.main_page_year) + comedyMoviesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + comedyMoviesModelClass.posterPath
            Picasso.get().load(pictureUrl).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(comedyMoviesModelClass) }
        }
    }
}