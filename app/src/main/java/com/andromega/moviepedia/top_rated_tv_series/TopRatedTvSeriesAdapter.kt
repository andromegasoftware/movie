...
class TopRatedTvSeriesAdapter (var movieList: List<TopRatedTvSeriesModelClass>, val clickListener: (TopRatedTvSeriesModelClass) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var circularMovies: List<TopRatedTvSeriesModelClass> = ArrayList()
    ...
    fun submitList(circularMovieList: List<TopRatedTvSeriesModelClass>){
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(topRatedTvSeriesModelClass: TopRatedTvSeriesModelClass, clickListener: (TopRatedTvSeriesModelClass) -> Unit){
            movieName.text = topRatedTvSeriesModelClass.MovieName
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + topRatedTvSeriesModelClass.picture
            Picasso.get().load(pictureUrl).error(R.drawable.avenger).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(topRatedTvSeriesModelClass) }
        }
    }
