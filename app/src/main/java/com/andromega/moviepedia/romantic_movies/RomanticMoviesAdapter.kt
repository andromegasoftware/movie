...
class RomanticMoviesAdapter (var movieList: List<RomanticMoviesModelClass>, val clickListener: (RomanticMoviesModelClass) -> Unit): RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var movies: List<RomanticMoviesModelClass> = ArrayList()
    ...
    fun submitList(movieList: List<RomanticMoviesModelClass>){
        movies = movieList
    }
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(romanticMoviesModelClass: RomanticMoviesModelClass, clickListener: (RomanticMoviesModelClass) -> Unit){
            movieName.text = romanticMoviesModelClass.title
            movieIdmb.text = itemView.resources.getString(R.string.average_vote) + romanticMoviesModelClass.voteAverage
            movieYear.text = itemView.resources.getString(R.string.main_page_year) + romanticMoviesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + romanticMoviesModelClass.posterPath
            Picasso.get().load(pictureUrl).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(romanticMoviesModelClass) }
        }
    }
}