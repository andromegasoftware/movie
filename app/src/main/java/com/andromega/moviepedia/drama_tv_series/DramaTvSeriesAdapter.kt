...
class DramaTvSeriesAdapter (var movieList: List<DramaTvSeriesModelClass>, val clickListener: (DramaTvSeriesModelClass) -> Unit) : RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var circularMovies: List<DramaTvSeriesModelClass> = ArrayList()
    ...
    fun submitList(circularMovieList: List<DramaTvSeriesModelClass>){
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(dramaTvSeriesModelClass: DramaTvSeriesModelClass, clickListener: (DramaTvSeriesModelClass) -> Unit){
            movieName.text = dramaTvSeriesModelClass.MovieName
            movieIdmb.text = itemView.resources.getString(R.string.average_vote) + dramaTvSeriesModelClass.voteAverage
            movieYear.text = itemView.resources.getString(R.string.main_page_year) + dramaTvSeriesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + dramaTvSeriesModelClass.picture
            Picasso.get().load(pictureUrl).error(R.drawable.avenger).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(dramaTvSeriesModelClass) }
        }
    }
}