...
class DocumentariesAdapter (var movieList: List<DocumentariesModelClass>, val clickListener: (DocumentariesModelClass) -> Unit): RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    private var movies: List<DocumentariesModelClass> = ArrayList()
    ...
    fun submitList(movieList: List<DocumentariesModelClass>){
        movies = movieList
    }
    ...
    class MoviesViewHolder constructor(itemView: View): RecyclerView.ViewHolder(itemView){
        ...
        fun bind(documentariesModelClass: DocumentariesModelClass, clickListener: (DocumentariesModelClass) -> Unit){
            movieName.text = documentariesModelClass.title
            movieIdmb.text = itemView.resources.getString(R.string.average_vote) + documentariesModelClass.voteAverage
            movieYear.text = itemView.resources.getString(R.string.main_page_year) + documentariesModelClass.releaseDate
            var pictureUrl = "https://image.tmdb.org/t/p/w500" + documentariesModelClass.posterPath
            Picasso.get().load(pictureUrl).into(movieImage)
            //Log.e("pictureUrl", pictureUrl)

            itemView.setOnClickListener { clickListener(documentariesModelClass) }
        }
    }
}